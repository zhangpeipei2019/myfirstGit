...
<script type="text/javascript">
	...
	$(function(){
		$('#treeGrid').treegrid({
			url:'auth?action=authTreeGridMenu&parentId=-1',
	...
	function openAuthAddDialog(){
		...
		url="auth?action=save&parentId="+node.id;
	}
	...
	function openAuthModifyDialog(){
		...
		url="auth?action=save&authId="+node.id;
	}
	...
	function deleteAuth(){
		...
		$.messager.confirm("系统提示","您确认要删除这个菜单节点吗?",function(r){
			if(r){
				$.post("auth?action=delete",{authId:node.id,parentId:parentNode.id},function(result){
					if(result.success){
